package com.dustdev.stormfarmazem.utils.misc.sql;
...
public class SQLProvider {
    ...
    public SQLConnector setup() {
        ...
        if(GeneralValue.get(GeneralValue::sqltype).equalsIgnoreCase("MYSQL")) {
            ConfigurationSection mysql = config.getConfigurationSection("MySQL");
            sqlConnector = mysqlDatabaseType(mysql).connect();
            Bukkit.getConsoleSender().sendMessage("§b[StormFArmazem] [MySQL]§f conexão com o MySQL efetuada com sucesso, criando tabelas...");
        } else if(GeneralValue.get(GeneralValue::sqltype).equalsIgnoreCase("SQLITE")) {
            ConfigurationSection sqlite = config.getConfigurationSection("SQLite");
            sqlConnector = sqliteDatabaseType(sqlite).connect();
            Bukkit.getConsoleSender().sendMessage("§b[StormFArmazem] [SQLite]§f conexão com o SQLite efetuada com sucesso, criando tabelas...");
        } else {
            return null;
        }
